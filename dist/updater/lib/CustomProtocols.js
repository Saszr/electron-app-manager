...
exports.registerHotLoadProtocol = exports.loadRemoteApp = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const url_1 = __importDefault(require("url"));
const downloader_1 = require("../lib/downloader");
const protocol_1 = __importDefault(require("../abstraction/protocol"));
...
function getZipUrl(_url) {
    return __awaiter(this, void 0, void 0, function* () {
        let result = yield downloader_1.request("HEAD", _url);
...
let cacheDir = path_1.default.join(process.cwd(), 'CACHE', 'Grid');
const getAppManagerForUrl = (repoUrl) => {
    // console.log('get app manager for', repoUrl)
    // console.log('create app manager with cache', cacheDir)
    return new AppManager_1.default({
        repository: repoUrl,
        auto: true,
        // FIXME we need to generate a subfolder folder for different apps for faster cache scan
        cacheDir: `${cacheDir}/${hashes_1.md5(repoUrl)}`
    });
};
const getAppManagerForVirtualHost = (hostname) => {
    if (!hosts[hostname]) {
        console.log('host not found', hostname, Object.keys(hosts));
    }
    return hosts[hostname];
};
exports.loadRemoteApp = (repoUrl, targetVersion, onProgress = (app, progress) => { }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let release = yield appManager.getLatestCached({
        version: targetVersion
    });
    // console.log('get latest cached that satisfies', targetVersion, release)
    if (release) {
        const appUrl = yield appManager._generateUrlForCachedRelease(release);
    ...
    release = yield appManager.getLatestRemote({
        version: targetVersion,
        download: true,
        downloadOptions: {
...
const showSplash = (handler, windowTitle, repoUrl, targetVersion = 'latest', service = 'GitHub') => __awaiter(void 0, void 0, void 0, function* () {
    let template = fs_1.default.readFileSync(__dirname + '/../electron/ui/splash.html', 'utf8');
...
const showSplashAndDetectWebContents = (handler, repoUrl, targetVersion) => new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    util_1.findWebContentsByTitle(windowTitle).then(webContents => {
    ...
    yield showSplash(handler, windowTitle, repoUrl, targetVersion);
}));
// url.parse was not working reliably so we just use this
// hack
const removeQuery = (_url) => {
...
const serveRequestFromCache = (hostname, pathname) => __awaiter(void 0, void 0, void 0, function* () {
...
const serveRequestFromCacheMem = util_1.memoize(serveRequestFromCache);
const hotLoadProtocolHandler = (request, handler) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const url_parts = url_1.default.parse(pseudoHttpRequest, true);
    let { hostname, pathname, query } = url_parts;
    const targetVersion = query && (typeof query.version === 'string') ? query.version : 'latest';
    ...
    const { webContents, emitUpdate } = yield showSplashAndDetectWebContents(handler, request, targetVersion);
    const appUrl = yield exports.loadRemoteApp(`https://${request}`, targetVersion, (app, progress) => {
 ...
 */
exports.registerHotLoadProtocol = (_cacheDir) => {
    if (isRegistered) {
        return `${scheme}:`;
    }
    cacheDir = _cacheDir ? _cacheDir : cacheDir;
    protocol_1.default.registerProtocolHandler(scheme, hotLoadProtocolHandler, (error) => {
        if (error)
